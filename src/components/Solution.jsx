...
const Solution = ({ scanCompleted, scanResults }) => {
  // State untuk testing - hanya untuk development/testing
  const [showTestControls] = useState(false);
  const [testSeverity] = useState('Parah');

  // Tentukan severity yang akan dipakai
  // Jika ada scanResults dan scan sudah completed, gunakan severity dari hasil scan
  // Jika tidak, gunakan testSeverity untuk testing (atau hide component)
  const getSeverityToUse = () => {
    if (scanCompleted && scanResults?.severity) {
      return scanResults.severity;
    }
    // Untuk testing/development, bisa return testSeverity
    // Untuk production, sebaiknya return null agar component tidak tampil
    return showTestControls ? testSeverity : null;
  };
  const severityToUse = getSeverityToUse();
  // Jika tidak ada severity (belum scan atau tidak ada hasil), jangan tampilkan apapun
  if (!severityToUse) {
    return null;
  }

  // Function untuk ambil data berdasarkan severity
  const getContentBySeverity = (severity) => {
    ...
    const contentMap = {
      ringan: {
        faktorPenyebab: {
          description:
            'Faktor ringan yang dapat menyebabkan iritasi kulit ringan pada kondisi Anda.',
          pills: [
      ...
      tinggi: {
        // Menambahkan mapping untuk "Tinggi" yang sesuai dengan API response
        faktorPenyebab: {
          description:
            'Faktor serius yang memperparah kondisi kulit Anda dan perlu perhatian khusus.',
          pills: [
      ...
      parah: {
        // Tetap pertahankan "parah" untuk backward compatibility
        faktorPenyebab: {
          description:
            'Faktor serius yang memperparah kondisi kulit Anda dan perlu perhatian khusus.',
          pills: [
    ...
    return contentMap[severityLevel] || contentMap.ringan;
  };
  ...
  const solutionSections = [
    {
      ...
      description: content.faktorPenyebab.description,
      icon: '🧬',
      pills: content.faktorPenyebab.pills,
      articlePath: 'faktor-penyebab',
    },
    {
      title: 'Gejala',
      description: content.gejala.description,
      icon: '🩺',
      pills: content.gejala.pills,
      articlePath: 'gejala',
    },
    {
      title: 'Rekomendasi Dosis',
      description: content.rekomendasiDosis.description,
      icon: '💊',
      pills: content.rekomendasiDosis.pills,
      articlePath: 'rekomendasi-dosis',
    },
    {
      title: 'Cara Mengatasi',
      description: content.caraMengatasi.description,
      icon: '🛡️',
      pills: content.caraMengatasi.pills,
      articlePath: 'cara-mengatasi',
    },
    {
      title: 'Tips',
      description: content.tips.description,
      icon: '💡',
      pills: content.tips.pills,
      articlePath: 'tips',
    },
    {
      title: 'Saran',
      description: content.saran.description,
      icon: '📋',
      pills: content.saran.pills,
      articlePath: 'saran',
    },
  ];
  ...
  // Function untuk menentukan warna berdasarkan severity
  const getSeverityColor = (severity) => {
    const severityLower = severity.toLowerCase();
    if (severityLower === 'tinggi' || severityLower === 'parah') {
      return 'red';
    ...
    <div className="max-w-6xl mx-auto mb-8 p-4">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="text-center mb-6">
          ...
          {scanCompleted && scanResults?.date && (
          ...
          {solutionSections.map((section, index) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {section.pills.slice(0, 4).map((pill, pillIndex) => (
                  <span
                    key={pillIndex}
                ...
                {section.pills.length > 4 && (
                  <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-200 text-gray-600">
                    +{section.pills.length - 4} lainnya
                  </span>
                )}
              </div>
            </div>
