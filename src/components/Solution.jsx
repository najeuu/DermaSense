...
const SeverityBadge = ({ severity, color, scanResults }) => (
  <div className="text-center mb-6">
    ...
    <div className="flex items-center justify-center gap-2">
      ...
      <span
        className={`px-3 py-1 rounded-full text-xs font-medium ${getSeverityColorClasses(color)}`}
      >
        {severity}
      </span>
    </div>
...
const PillList = ({ pills }) => (
  <div className="flex flex-wrap gap-2">
    {pills.map((pill, index) => (
      <span key={index} className={`px-2 py-1 rounded-full text-xs font-medium ${pill.color}`}>
        {pill.text}
      </span>
    ))}
  </div>
  ...
  return (
    <div className="max-w-6xl mx-auto mb-8 p-4">
      <div className="bg-white rounded-lg shadow-lg p-6">
        <SeverityBadge severity={severityToUse} color={severityColor} scanResults={scanResults} />
